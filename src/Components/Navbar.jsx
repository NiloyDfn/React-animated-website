...
function Navbar() {
  ...
  return (
    <nav className={`w-full fixed z-[99] flex items-center justify-between py-4 md:py-6 lg:py-8 px-4 md:px-10 lg:px-20 font-['Neue_Montreal'] transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-sm shadow-lg' : 'bg-transparent'}`}>
      {/* Logo */}
      <div className="logo">
      ...
      <div className="hidden md:flex gap-6 lg:gap-10">
        {navLinks.map((item, index) => (
          <a 
            ...
            className={`text-sm lg:text-md font-light text-white hover:text-gray-300 transition-colors ${index === 4 && "lg:ml-32"} capitalize`} 
            key={index}
      ...
      <div 
        className={`fixed top-0 right-0 h-full w-64 bg-black/95 backdrop-blur-md transform transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="flex flex-col p-6 pt-10">
          {navLinks.map((item, index) => (
            <a 
              href={`#${item.toLowerCase().replace(/\s+/g, '-')}`} 
              className="text-white text-lg py-4 hover:text-gray-300 transition-colors capitalize border-b border-gray-800" 
              key={index}
              onClick={handleLinkClick}
            >
              {item}
